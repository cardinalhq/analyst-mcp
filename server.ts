...
type VizSuggestion = {
  ...
  fields?: {                 // optional auto-detected field roles
    measures?: string[];
    dimensions?: string[];
    time?: string;
    aggregation?: string;
...
registerTool({
  ...
  handler: async ({ profileId, rows, dataset, sql, question, prefer, maxSeries, maxRows }): Promise<VizSuggestion> => {
    ...
    const payload: any = { profileId, rows, dataset, sql, question, prefer, maxSeries, maxRows };
...
registerTool({
    ...
    description:
        'Analyze SQL results and return an optimal chart type (bar, line, pie, scatter) with configuration for frontend visualization. If the user specifies a preferred chart type, that preference takes priority.',
    inputSchema: {
        type: 'object',
        required: ['data'],
        properties: {
    ...
    handler: async ({ data, preferredChart }) => {
        ...
        if (rows.length === 0) return { chartType: 'table', config: {} };
        ...
        const config = {
            labels:
                categoricalCols.length > 0
                    ? rows.map((r: any) => r[categoricalCols[0]])
                    : rows.map((_r: any, i: number) => i + 1),
            datasets: numericCols.map((col) => ({
                label: col,
                data: rows.map((r: any) => r[col]),
            })),
        };
